...
{
    /// <summary>
    /// Per-player wardrobe: save, dress, and delete up to five build presets.
    /// </summary>
    public sealed class WardrobeBuildScreen : MonoBehaviour
    {
        ...
        private void Build()
        {
            ...
            boxRt.anchorMin = new Vector2(0.14f, 0.1f);
            boxRt.anchorMax = new Vector2(0.86f, 0.9f);
            ...
            _title = UiFactory.AddText(box.transform, "Title", "WARDROBE", 30, TextAnchor.MiddleCenter, Color.white,
                new Vector2(0.05f, 0.9f), new Vector2(0.95f, 0.98f), Vector2.zero, Vector2.zero, UiFonts.MenuBold);

            _status = UiFactory.AddText(box.transform, "Status", string.Empty, 15, TextAnchor.MiddleCenter,
                new Color(0.75f, 0.78f, 0.85f, 1f),
                new Vector2(0.05f, 0.04f), new Vector2(0.58f, 0.1f), Vector2.zero, Vector2.zero, UiFonts.MenuRegular);
            ...
            var close = UiFactory.AddButton(box.transform, "Close", "DONE",
                new Vector2(0.62f, 0.04f), new Vector2(0.94f, 0.1f), Vector2.zero, Vector2.zero,
        ...
        private void AddSlotRow(Transform parent, int slotIndex, ref float yTop)
        {
            ...
            UiFactory.AddText(parent, $"Slot{slotIndex}Label", $"SLOT {slotIndex + 1}", 16, TextAnchor.UpperLeft,
                new Color(0.82f, 0.78f, 0.95f, 1f),
                new Vector2(0.05f, yMin + 0.04f), new Vector2(0.2f, yTop), Vector2.zero, Vector2.zero, UiFonts.MenuBold);

            var summary = UiFactory.AddText(parent, $"Slot{slotIndex}Summary", "(Empty)", 14, TextAnchor.UpperLeft,
                Color.white, new Vector2(0.2f, yMin), new Vector2(0.56f, yTop), Vector2.zero, Vector2.zero, UiFonts.MenuRegular);
            ...
            save.GetComponentInChildren<Text>().fontSize = 14;
            ...
            dress.GetComponentInChildren<Text>().fontSize = 14;
            ...
            delete.GetComponentInChildren<Text>().fontSize = 14;
        ...
        {
            _confirm.Show(
                $"Save your current spells and item to Slot {slotIndex + 1}? This will overwrite any build stored there.",
                () =>
                {
        ...
        private void PromptDress(int slotIndex)
        {
            ...
            var preset = PlayerBuildService.GetPreset(ctx.Save.Current.meta, _playerIndex, slotIndex);
            if (preset.IsEmpty)
            {
            ...
            _confirm.Show(
                $"Equip the build saved in Slot {slotIndex + 1}? Your current lobby loadout will be replaced.",
                () =>
                {
                    PlayerBuildService.DressBuild(_playerIndex, slotIndex, ctx.Save);
                    Refresh();
                });
        }
        ...
        private void PromptDelete(int slotIndex)
        {
            ...
            _confirm.Show(
                $"Delete the build in Slot {slotIndex + 1}? This cannot be undone.",
                () =>
                {
                    PlayerBuildService.DeleteBuild(_playerIndex, slotIndex, ctx.Save);
                    Refresh();
                });
        }
        ...
        private void Refresh()
        {
            ...
            _title.text = $"WARDROBE — P{_playerIndex + 1}";
            _status.text = "Each slot stores 4 spells + 1 item from your current lobby loadout.";

            for (int i = 0; i < _slotSummaries.Count && i < PlayerBuildService.SlotCount; i++)
            {
                var preset = PlayerBuildService.GetPreset(meta, _playerIndex, i);
